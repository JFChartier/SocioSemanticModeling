...
```{r}
lda_simil_members=text2vec::sim2(as.matrix(agg.topic.distr.BySender[,2:ncol(agg.topic.distr.BySender)]), method = "cosine", norm = "none")
```

###save
```{r}
save(lda_simil_members, file="enron.lda.sim.member.rds")
```
...
##Similarity calculation
```{r}
lsa_simil_members=text2vec::sim2(as.matrix(agg.latentDim.BySender[,2:ncol(agg.latentDim.BySender)]), method = "cosine", norm = "none")
```

###save
```{r}
save(lsa_simil_members, file="enron.lsa.sim.member.rds")
```
...
<!-- ```{r} -->
<!-- library(nnTensor) -->
<!-- nnmf.model=nnTensor::NMF(message.term.matrix.filtered.wd[1:1000,]%>%dfm_weight(x=., scheme="prop")%>%as.matrix(.), J = 100) -->
...
<!-- dummy=c(dummy.feature=c(rep(0.0001, nrow(message.term.matrix.filtered)))) -->

<!-- message.term.matrix.filtered.wd=message.term.matrix.filtered%>%Matrix::cBind(., dummy)%>%as.dfm(.) -->

<!-- set.seed(12345) -->

<!-- #init <- NMF::nmfModel(k, matrixContextWordNorme, W=t(kmeansModelOnCol$centers), H=kmeansModelOnRow$centers) -->

<!-- nnmf.model.snmf = NMF::nmf(message.term.matrix.filtered.wd[1:1000,]%>%dfm_weight(x=., scheme="prop") %>% as.matrix(.), rank=100, method = "brunet", .options=list('v'=T, 'P'=T), seed=1) -->


<!-- #calculs des similarités avec la métrique du cosinus entre mots et mixture coefficient -->
<!-- simil_Mot_mixtureCoefficient=proxy::simil(x=t(matrixContextWordNorme), y = t(basis(nonNegResult)), by_rows=T, method="cosine", convert_distances = FALSE) -->


<!-- #Afficher résultats -->
<!-- View(k_plus_important_mot_par_topic(15, simil_Mot_mixtureCoefficient), title = "Proximités entre mots et axes dans un modèle NMF") -->
